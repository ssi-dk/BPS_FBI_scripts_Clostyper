...
rule trsanposons_search_with_srst2:
    ...
    shell:
        "srun -p project --mem 10000 -c {params.cores} srst2 --input_pe {input.r1} {input.r2} --output {params.trsanposons_dir}srst2 --log --gene_db {transposon_db_dir}{transposon_db_name} --threads {threads} --min_coverage {transposon_coverage} --stop_after 1000000 --report_new_consensus --report_all_consensus "
...
rule compile_transposons:
    ...
    conda:
        envs_folder + "srst2.yaml"
    shell:
        "srun -p project --mem 10000 -c {params.cores} srst2 --prev_output {input.trsanposons_results_per_sample} --output {output.trsanposons_results_all_sample}"
...
rule plasmids_search_with_srst2:
    ...
    shell:
        "srun -p project --mem 10000 -c {params.cores} srst2 --input_pe {input.r1} {input.r2} --output {params.plasmids_dir}srst2 --log --gene_db {plasmids_db_dir}{plasmids_db_name} --threads {threads} --min_coverage {plasmids_coverage} --stop_after 1000000 --report_new_consensus --report_all_consensus "
        "&& if [[ -f {params.srst2_raw_summary} ]]; then  mv {params.srst2_raw_summary} {output.srst2_summary}; else touch {output.srst2_summary}; fi " #incase of negative results create empty file
        "&& if [[ -f {params.srst2_raw_summary_full} ]]; then mv {params.srst2_raw_summary_full} {output.srst2_fullsummary}; else touch {output.srst2_fullsummary}; fi"
...
rule phages_search_with_srst2:
    ...
    shell:
        "srun -p project --mem 10000 -c {params.cores} srst2 --input_pe {input.r1} {input.r2} --output {params.phages_dir}srst2 --log --gene_db {phages_db_dir}{phages_db_name} --threads {threads} --min_coverage {phages_coverage} --stop_after 1000000 --report_new_consensus --report_all_consensus "
        "&& if [[ -f {params.srst2_raw_summary} ]]; then  mv {params.srst2_raw_summary} {output.srst2_summary}; else touch {output.srst2_summary}; fi " #incase of negative results create empty file
        "&& if [[ -f {params.srst2_raw_summary_full} ]]; then mv {params.srst2_raw_summary_full} {output.srst2_fullsummary}; else touch {output.srst2_fullsummary}; fi"
...
rule resistance_mutations_positions_calling:
    ...
    shell:
        ...
        "srun -p project --mem 10000 -c {params.cores} bowtie2-build --quiet {params.REF} {params.REF}"
        " && srun -p project --mem 10000 -c {params.cores} samtools faidx {params.REF} "
        #align the short reads with mode very-sensitive-local
        " && mkdir -p {params.resistance_dir}"
        " && srun -p project --mem 10000 -c {params.cores} bowtie2 --quiet -1 {input.r1} -2 {input.r2} -S {output.sam} -q --very-sensitive-local --no-unal -a -x {params.REF} --threads {threads} -u 1000000"
        #make the BAM file
        " && srun -p project --mem 10000 -c {params.cores} samtools view -@ {threads} -b -O BAM -o {output.unsortedbam} -q 1 -S {output.sam}"
        " && srun -p project --mem 10000 -c {params.cores} samtools sort -@ {threads} -O BAM {output.unsortedbam} -o {output.sortedbam}"
        #produce a vcf file from the BAM file & index it
        #https://www.biostars.org/p/367960/
        " && srun -p project --mem 10000 -c {params.cores} bcftools mpileup -Ou -f {params.REF} {output.sortedbam} | bcftools call -Ou --variants-only --multiallelic-caller | bcftools norm -f {params.REF} -Oz -o {output.vcf} "
        " && srun -p project --mem 10000 -c {params.cores} tabix  {output.vcf}"
        #create a consensus sequence (ref seq seeded with sample variants)
        " && srun -p project --mem 10000 -c {params.cores} bcftools consensus --fasta-ref {params.REF} --haplotype 1 {output.vcf} > {output.consensus}"
        ...
        " && srun -p project --mem 10000 -c {params.cores} seqkit translate --seq-type dna --transl-table 11 {output.consensus}  --out-file {output.consensus_aa}"
        " && srun -p project --mem 10000 -c {params.cores} seqkit subseq --bed {params.aminoacid_positions} {output.consensus_aa} | seqkit fx2tab > {output.aminoacid_change_summary}"
        " && srun -p project --mem 10000 -c {params.cores} bcftools mpileup -T {params.nucleotide_positions} -Ou -f {params.REF}  {output.sortedbam} | bcftools call -Ou -vm | bcftools norm -f {params.REF} -Oz | bcftools query -f '%CHROM\\t%POS\\t%REF\\t%ALT\\n' --print-header -o {output.nucleotide_change_summary}"
#===================================================
